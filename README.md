...
**Svelte 5 + SvelteKit 2.63 or 3. Data layer only — bring your own rendering.**
...
v2 declares **Svelte >= 5.39** (experimental async SSR) and **@sveltejs/kit >= 2.63 or >= 3** (`$app/state`, `$app/env`, `$app/paths` `resolve`). SvelteKit 3 removed the `base` export from `$app/paths` and deprecated `$app/environment`, so base-path handling goes through `resolve()` (kit 2.26+) and environment flags come from `$app/env` (kit 2.63+).
...
- **SvelteKit >= 2.63, or >= 3** — relies on `$app/state`, `$app/env`, `import.meta.glob`, and `resolve()` from `$app/paths`
